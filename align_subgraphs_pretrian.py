...
from rule_based.subgraph_rules import is_subgraph, fuzzy_align_subgraphs, postprocess_subgraph, clean_subgraph
...
def main():
    ...
    for amr in tqdm(amrs, file=sys.stdout):
        ...
        if amr.id in node_alignments:
            ...
            fuzzy_align_subgraphs(amr, new_alignments, english=ENGLISH)
            for align in new_alignments:
                if not align: continue
                postprocess_subgraph(amr, subgraph_alignments, align, english=ENGLISH)
    ...
    display_file = amr_file.replace('.txt', '') + '.subgraphs.pretrain.html'
    ...
    align_file = amr_file.replace('.txt', '') + f'.subgraph_alignments.pretrain.json'
