...
def rule_based_align_relations(amr, subgraph_alignments, relation_alignments):
    ...
    for s,r,t in amr.edges:
        ...
        if s in salign.nodes and t in salign.nodes:
            add_relation_alignment(amr, relation_alignments, (s, r, t), salign.tokens)
        elif any(r.startswith(prefix) for prefix in [':ARG',':op',':snt']) and not r.endswith('-of'):
            add_relation_alignment(amr, relation_alignments, (s,r,t), salign.tokens)
        elif r in [':domain']:
            add_relation_alignment(amr, relation_alignments, (s,r,t), salign.tokens)
...
def exact_match_relations(amr, subgraph_alignments, relation_alignments):
    for s, r, t in amr.edges:
        if r.startswith(':prep-'):
            ...
            token_labels = ["'s", 'of']
            candidate_spans = [span for span in amr.spans if ' '.join(amr.lemmas[t].lower() for t in span) in token_labels]
            candidate_spans = [span for span in candidate_spans if not amr.get_alignment(subgraph_alignments, token_id=span[0])]
            if len(candidate_spans) == 1:
                add_relation_alignment(amr, relation_alignments, (s, r, t), candidate_spans[0])
